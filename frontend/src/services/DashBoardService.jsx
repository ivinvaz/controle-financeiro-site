import { listar as listarMetas } from "./MetaService";
import { consultarTodos as listarTransacao } from "./transacaoServices";
...
{
    const PALETA_CORES = ["#4ade80", "#f87171", "#60a5fa", "#fbbf24", "#a78bfa", "#34d399", "#f472b6", "#94a3b8"];
    return PALETA_CORES[indice % PALETA_CORES.length];
}

async function GetTransacoesDate(DataInicio = null, DataFim = null)
{
    const transacoes = (await listarTransacao()) ?? [];
...
async function GetMetasDate(DataInicio = null, DataFim = null)
{
    ...
    else if (DataInicio !== null && DataFim === null)
    {
        ...
        metasFiltradas = metas.filter((meta) => {
            return (meta.ano > anoInicio) || (meta.ano === anoInicio && meta.mes >= mesInicio);
        });
    }
    else if (DataInicio === null && DataFim !== null)
    {
    ...
    else
    {
        ...
        metasFiltradas = metas.filter((meta) => {
            const depoisDoInicio = (meta.ano > anoInicio) || (meta.ano === anoInicio && meta.mes >= mesInicio);
            const antesDoFim = (meta.ano < anoFim) || (meta.ano === anoFim && meta.mes <= mesFim);
...
async function GetSumReceitas(DataInicio = null, DataFim = null) 
{
  ...
  const transacoesReceita = transacoes.filter((transacao) => transacao.natureza === "receita");
...
async function GetSumDespesas(DataInicio = null, DataFim = null) 
{
  ...
  const transacoesDespesa = transacoes.filter((transacao) => transacao.natureza === "despesa");
...
async function GetDashboardData({ DataInicio = null, DataFim = null, receitaAtiva = true, despesaAtiva = true } = {})
{
    ...
    const [todasTransacoes, todasMetas] = await Promise.all([GetTransacoesDate(DataInicio, DataFim),GetMetasDate(DataInicio, DataFim),]);
    ...
    const transacoesReceita = transacoesFiltradas.filter((transacao) => transacao.natureza === "receita");
    const transacoesDespesa = transacoesFiltradas.filter((transacao) => transacao.natureza === "despesa");
    ...
    if (ambosAtivos)
    {
        ...
        datasetsLinha.push(
            { label: "Receitas", data: receitaPorDia, borderColor: "#4ade80", backgroundColor: "#4ade80" },
            { label: "Despesas", data: despesaPorDia, borderColor: "#f87171", backgroundColor: "#f87171" },
        );
    }
    else
    {
...
export { GetSumDespesas, GetSumMetas, GetSumReceitas, GetMetasDate, GetTransacoesDate, GetDashboardData }