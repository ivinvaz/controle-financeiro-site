...
function CadastroMeta() {
  ...
  const onSubmit = async (data) => {
    ...
    const mesIndex = MESES.indexOf(data.mes) + 1;

    const resultado = await MetaService.criar({
      nome: data.nome.trim(),
      mes: mesIndex > 0 ? mesIndex : data.mes,
      ano: Number(data.ano),
    ...
    if (resultado.success) {
      setSubmitSuccess(resultado.message || "Meta criada com sucesso.");
      setTimeout(() => navigate("/metas"), 1200);
    } else {
      setSubmitError(resultado.message || "Erro ao criar meta.");
    }
  };
